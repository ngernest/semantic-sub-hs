...
genMetafunctionSpec
  ...
  rawRngTy = do
    describe "First Projection Tests" $ do
      it "Projection from Empty 1" $ fstProjEquiv Empty Empty `shouldBe` True
      it "Projection from Empty 2" $ fstProjEquiv (And [Prod (Or [T, Z]) F, Not (Prod Z Any)]) T `shouldBe` True
      it "Simple Pair" $
        property $
        ...
        property $
          \t1 t2 ->
            fstProjEquiv
              ...
              (Or [t1, t2])
      it "Intersection of Pairs" $
        property $
          \t1 t2 ->
            fstProjEquiv
              (And [Prod t1 Any, Prod t2 Any])
              (And [t1, t2])
      it "Intersection of Union of Pairs1" $
        property $
          \t1 t2 t3 t4 ->
            fstProjEquiv
              ( And
      ...
      it "Intersection of Union of Pairs2" $
        property $
      ...
      it "Union of Intersection of Pairs1" $
        property $
          \t1 t2 t3 t4 ->
            fstProjEquiv
              ( Or
      ...
      it "Union of Intersection of Pairs2" $
        property $
          \t1 t2 t3 t4 t5 t6 t7 t8 ->
            fstProjSubtype
              ( Or
    ...
    describe "Second Projection Tests" $ do
      it "Projection from Empty 1" $ sndProjEquiv Empty Empty `shouldBe` True
      it "Projection from Empty 2" $
    ...
    describe "Function Range Tests" $ do
      it "Simple Arrow1" $ rngTyEquiv (Arrow T F) T F `shouldBe` True
      it "Simple Arrow2" $
        rngTyEquiv
          ( And
              [ Arrow (Or [T, F]) F,
