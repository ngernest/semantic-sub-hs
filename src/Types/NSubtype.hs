...
import Common.SetOps (nonEmptySubsets, subsets)
import Data.List ((\\))
...
flattenProds prods = flattenAux anyTy anyTy [] prods
  where
    flattenAux :: Ty -> Ty -> [Prod] -> BDD Prod -> [(Prod, [Prod])]
...
isEmptyArrow as = all emptyClause (flattenBDD as)
  where
    ...
    emptyClause (pos, neg) = any (emptyArrow dom pos) neg
      where
        dom = foldl (\t (Arrow s1 _) -> tyOr t s1) emptyTy pos
    ...
    emptyHelper t1 t2 pos pos' =
      ...
      where
        dom =
          ( foldl
              (\t (Arrow s1 _) -> tyOr t s1)
              emptyTy
              (pos \\ pos')
          )
        rng =
          ( foldl
              (\t (Arrow _ s2) -> tyAnd t s2)
              anyTy
              pos'
          )
