...
fstProj :: Ty -> Maybe Ty
fstProj = calcProj (\t1 t2 -> t1)

-- If t is a product, what type is returned
-- from it's second projection. If it is not
-- a product, return Nothing.
sndProj :: Ty -> Maybe Ty
sndProj = calcProj (\t1 t2 -> t2)
...
domTy t
  ...
  | otherwise =
      let (Ty _ _ arrows) = t
       in Just (aux anyTy emptyTy arrows)
  where
    aux :: Ty -> Ty -> BDD Arrow -> Ty
    aux acc dom Top = tyAnd acc dom
    aux acc dom Bot = acc
    aux acc dom (Node (Arrow t _) l m r) = acc3
      where
        acc1 = aux acc (tyOr dom t) l
        acc2 = aux acc1 dom m
        acc3 = aux acc2 dom r
